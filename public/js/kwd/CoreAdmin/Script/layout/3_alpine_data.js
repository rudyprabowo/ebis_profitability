document.addEventListener('alpine:init', () => {
    Alpine.data('table', () => ({
        ...
        bind: {
            ...
            row_checkbox: {
                ['@click']() {
                    ...
                    let checked = this.$el.checked;
                    const layoutStore = Alpine.store('layout');
                    layoutStore.changeCheck(dataset.idx, checked);
        ...
        changeStatus(idx) {
            ...
            let btn = "Activate Layout";
            if (!this.$el.checked) {
                act = "<b class='text-red-700'>deactivate</b>";
                btn = "Deactivate Layout";
            }
            this.$el.checked = old_val;
            const confirmStore = Alpine.store('confirm_modal');
            const layoutStore = Alpine.store('layout');
            let module = layoutStore.getCurrentDataByIndex(idx);
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Layout ${module.id} - ${module.name}</b> ?`, "", "info", `${btn}`,
                () => {
                    this.$el.checked = !old_val;
                    confirmStore.hide();
        ...
        removeRow(idx) {
            ...
            const confirmStore = Alpine.store('confirm_modal');
            const layoutStore = Alpine.store('layout');
            let layout = layoutStore.getCurrentDataByIndex(idx);
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Layout ${layout.id} - ${layout.name}</b> ?`, "", "remove", `${btn}`,
                () => {
                    confirmStore.hide();
                    removeLayout(layout);
                }
        ...
        removeAllRow(idx) {
            // console.log(idx, this.$el.checked);
            let act = "<b class='text-red-700'>remove</b>";
            let btn = "Remove Selected Layout";

            const layoutStore = Alpine.store('layout');
            let modules = layoutStore.getSelectedController();
            // console.table(modules);
            if (modules.length > 0) {
                const confirmStore = Alpine.store('confirm_modal');
                confirmStore.show(
                    `${btn}`, `Are you sure you want to ${act} <b>Selected Layout</b> ?`, "", "remove", `${btn}`,
                    () => {
                        confirmStore.hide();
                        removeMultiLayout(modules.map((v, k) => { return v.id }));
                    }
                );
            }
        }
    ...
    Alpine.data('create', () => ({
        ...
        submitCreate() {
            ...
            if (current_data.name === "" || current_data.name === null) {
                this.name_msg = "Layout name can not be null";
            } else {
                if (current_data.id !== null) {
                    updateLayout(current_data);
                } else {
                    current_data = _.pickBy(current_data, (val, idx) => {
                        return idx !== "id";
                    });
                    // console.log(current_data);
                    addLayout(current_data);
                }
            }
        },
        cancelCreate() {
        ...
        showEdit(idx) {
            const module_dataStore = Alpine.store('layout');

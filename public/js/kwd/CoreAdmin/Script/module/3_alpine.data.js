document.addEventListener('alpine:init', () => {
  Alpine.data('table', () => ({
    ...
    bind: {
      ...
      col_status: {
        ['@click']() {
          // console.log(this.$el.dataset.idx);
      ...
      row_checkbox: {
        ['@click']() {
          ...
          let checked = this.$el.checked;
          const moduleStore = Alpine.store('module');
          moduleStore.changeCheck(dataset.idx,checked);
    ...
    changeStatus(idx) {
      ...
      let checkbox = this.$el.children[0];
      // console.log(idx, this.$el.checked);
      let old_val = !checkbox.checked;
      let act = "<b class='text-green-700'>activate</b>";
      let btn = "Activate Module";
      if (!checkbox.checked) {
        act = "<b class='text-red-700'>deactivate</b>";
        btn = "Deactivate Module";
      }
      checkbox.checked = old_val;
      const confirmStore = Alpine.store('confirm_modal');
      const moduleStore = Alpine.store('module');
      let module = moduleStore.getCurrentDataByIndex(idx);
      confirmStore.show(
        `${btn}`, `Are you sure you want to ${act} <b>Module ${module.id} - ${module.name}</b> ?`, "", "info", `${btn}`,
        () => {
          // checkbox.checked = !old_val;
          confirmStore.hide();
          changeStatus(module,checkbox, old_val);
        }
      );
    },
    removeRow(idx) {
      ...
      let btn = "Remove Module";
      const confirmStore = Alpine.store('confirm_modal');
      const moduleStore = Alpine.store('module');
      let module = moduleStore.getCurrentDataByIndex(idx);
      confirmStore.show(
        `${btn}`, `Are you sure you want to ${act} <b>Module ${module.id} - ${module.name}</b> ?`, "", "remove", `${btn}`,
        () => {
          confirmStore.hide();
          removeModule(module);
        }
    ...
    removeAllRow(idx) {
      // console.log(idx, this.$el.checked);
      let act = "<b class='text-red-700'>remove</b>";
      let btn = "Remove Selected Module";

      const moduleStore = Alpine.store('module');
      let modules = moduleStore.getSelectedModule();
      // console.table(modules);
      if (modules.length > 0) {
        const confirmStore = Alpine.store('confirm_modal');
        confirmStore.show(
          `${btn}`, `Are you sure you want to ${act} <b>Selected Module</b> ?`, "", "remove", `${btn}`,
          () => {
            confirmStore.hide();
  ...
  Alpine.data('create', () => ({
    ...
    submitCreate() {
      ...
      if (current_data.name === "" || current_data.name === null) {
        this.name_msg = "Module name can not be null";
      } else {
        if (current_data.id !== null) {
          updateModule(current_data);
        } else {
          current_data = _.pickBy(current_data, (val, idx) => {
            return idx !== "id";
          });
          // console.log(current_data);
          addModule(current_data);
        }
      }
    },
    cancelCreate() {
    ...
    showEdit(idx) {
      const module_dataStore = Alpine.store('module');
      let _data = _.cloneDeep(module_dataStore.getCurrentDataByIndex(idx));
      console.table(_data);
      this.title = "Edit Module - "+_data.id;
      this.data = {
        id: _data.id,
        name: _data.name,
  ...
  Alpine.data('upload', () => ({
    ...
    ajax: {
      url: "/core-admin/manage-script/module/upload",
      method: "POST",

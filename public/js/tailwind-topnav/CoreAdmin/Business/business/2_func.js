function reloadData() {
    ...
    console.log("TES");
    fetchWithTimeout(
        "/core-admin/manage-business/business/getall?dbcache=0",
        {
            // mode: "no-cors",
            // redirect: "follow",
            method: 'POST',
...
function addBusiness(param) {
    const mainLoader = Alpine.store('loader');
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/manage-business/business/create",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                ...
                window.dispatchEvent(new CustomEvent('add-notif', {
                    detail: { title: "<span class='text-green-700'>Add Business Unit</span>", msg: `Success add <b>Business Unit ${param.name}</b>`, timeout: 5000 }
                }));
                // window.dispatchEvent(new CustomEvent('success-create'));
                // const notifStore = Alpine.store('notif_modal');
                ...
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Add Business Unit`, `Failed <b>add Business Unit</b>, please try again`, "", "failed", `CLOSE`, () => {
                    notifStore.hide();
                });
            }
        })
...
function updatebusiness(param) {
    // console.log("update");

    const mainLoader = Alpine.store('loader');
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/manage-business/business/update",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                window.dispatchEvent(new CustomEvent('success-create'));
                window.dispatchEvent(new CustomEvent('add-notif', {
                    detail: { title: "<span class='text-indigo-700'>Update business</span>", msg: `Success update <b>business ${param.id} - ${param.name}</b>`, timeout: 5000 }
                }));
                // const notifStore = Alpine.store('notif_modal');
                ...
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Update business`, `Failed <b>update business</b>, please try again`, "", "failed", `CLOSE`, () => {
                    notifStore.hide();
                });
            }
        })
...
function removebusiness(param) {
    const mainLoader = Alpine.store('loader');
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/manage-business/business/delete",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                ...
                filterData();
                window.dispatchEvent(new CustomEvent('add-notif', {
                    detail: { title: "<span class='text-red-700'>Remove business</span>", msg: `Success remove  <b>business ${param.id} - ${param.name}</b>`, timeout: 5000 }
                }));
                // const notifStore = Alpine.store('notif_modal');
                ...
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Remove business`, `Failed <b>remove business</b>, please try again`, "", "failed", `CLOSE`, () => {
                    notifStore.hide();
                });
            }
        })
...
function removeMultibusiness(id) {
    // console.log(id);
    const mainLoader = Alpine.store('loader');
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/manage-business/business/deletemulti",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                ...
                filterData();
                window.dispatchEvent(new CustomEvent('add-notif', {
                    detail: { title: "<span class='text-red-700'>Remove business</span>", msg: `Success <b>remove selected business</b>`, timeout: 5000 }
                }));
                // const notifStore = Alpine.store('notif_modal');
                // notifStore.show(`Remove Module`, `Success <b>remove module</b>`, "", "success", `CLOSE`, () => {
                //   notifStore.hide();
...
function updatestatusbusiness(param) {
    // console.log("update");

    const mainLoader = Alpine.store('loader');
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/manage-business/business/updatestatus",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                window.dispatchEvent(new CustomEvent('success-create'));
                window.dispatchEvent(new CustomEvent('add-notif', {
                    detail: { title: "<span class='text-indigo-700'>Update status business</span>", msg: `Success update <b>status business ${param.id} - ${param.name}</b>`, timeout: 5000 }
                }));
                // const notifStore = Alpine.store('notif_modal');
                // notifStore.show(`Update Module`, `Success <b>update module</b>`, "", "success", `CLOSE`, () => {
                //   notifStore.hide();
                ...
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Update status business`, `Failed <b>update status business</b>, please try again`, "", "failed", `CLOSE`, () => {
                    notifStore.hide();
                });
            }
        })

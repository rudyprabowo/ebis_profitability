document.addEventListener('alpine:init', () => {
    Alpine.data('table', () => ({
        ...
        changeStatus(idx) {
            ...
            let btn = "Activate Business";
            if (!this.$el.checked) {
                act = "<b class='text-red-700'>deactivate</b>";
                btn = "Deactivate Business";
            }
            ...
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Business ${role.id} - ${role.name}</b> ?`, "", "info", `${btn}`,
                () => {
                    this.$el.checked = !old_val;
                    confirmStore.hide();
        ...
        removeRow(idx) {
            ...
            let btn = "Remove Business";
            const confirmStore = Alpine.store('confirm_modal');
            const businessStore = Alpine.store('business');
            let role = businessStore.getCurrentDataByIndex(idx);
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Business ${role.id} - ${role.name}</b> ?`, "", "remove", `${btn}`,
                () => {
                    confirmStore.hide();
                    removebusiness(role);
                }
        ...
        removeAllRow(idx) {
            // console.log(idx, this.$el.checked);
            let act = "<b class='text-red-700'>remove</b>";
            let btn = "Remove Selected Business";
            ...
            if (role.length > 0) {
                const confirmStore = Alpine.store('confirm_modal');
                confirmStore.show(
                    `${btn}`, `Are you sure you want to ${act} <b>Selected Business</b> ?`, "", "remove", `${btn}`,
                    () => {
                        confirmStore.hide();
                        removeMultibusiness(role.map((v, k) => { return v.id }));
                    }
                );
            }
        }
    ...
    Alpine.data('create', () => ({
        ...
        submitCreate() {
            ...
            if (current_data.redirect_route === "--") {
                current_data.redirect_route = null;
            }
            ...
            if (current_data.code === "" || current_data.code === null) {
                ...
                if (current_data.id !== null) {
                    updatebusiness(current_data);
                } else {
                    current_data = _.pickBy(current_data, (val, idx) => {
                        return idx !== "id";
                    });
                    console.log(current_data);
                    addBusiness(current_data);
                }
            }
        },
        cancelCreate() {
        ...
        showEdit(idx) {
            ...
            // console.table(_data);
            this.title = "Edit Busines Unit - " + _data.id;
            this.data = {
                id: _data.id,
                name: _data.name,

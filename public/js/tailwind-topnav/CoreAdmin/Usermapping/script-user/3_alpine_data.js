document.addEventListener('alpine:init', () => {
    Alpine.data('table', () => ({
        ...
        bind: {
            ...
            check_all: {
                ['@click']() {
                    ...
                    const layoutStore = Alpine.store('script_user');
        ...
        changeStatus(idx) {
            ...
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Script ${role.id} - ${role.id}</b> ?`, "", "info", `${btn}`,
                () => {
                    this.$el.checked = !old_val;
                    confirmStore.hide();
        ...
        removeRow(idx) {
            ...
            let role = roleStore.getCurrentDataByIndex(idx);
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Script ${role.id} - ${role.id}</b> ?`, "", "remove", `${btn}`,
                () => {
                    confirmStore.hide();
                    removeScript(role);
                }
    ...
    Alpine.data('create', () => ({
        ...
        showEdit(idx) {
            const role_dataStore = Alpine.store('script_user');
            let _data = _.cloneDeep(role_dataStore.getCurrentDataByIndex(idx));
            // console.table(_data);
            this.title = "Edit Script - " + _data.id;
            this.data = {
                action: _data.action,

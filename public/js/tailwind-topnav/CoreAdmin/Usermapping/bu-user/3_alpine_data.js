document.addEventListener('alpine:init', () => {
    Alpine.data('table', () => ({
        ...
        bind: {
            ...
            check_all: {
                ['@click']() {
                    ...
                    const layoutStore = Alpine.store('bu_user');
        ...
        changeStatus(idx) {
            ...
            let btn = "Activate Business Unit";
            if (!this.$el.checked) {
                act = "<b class='text-red-700'>deactivate</b>";
                btn = "Deactivate Business Unit";
            }
            ...
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Business Unit ${role.id} - ${role.id}</b> ?`, "", "info", `${btn}`,
                () => {
                    this.$el.checked = !old_val;
                    confirmStore.hide();
        ...
        removeRow(idx) {
            ...
            let btn = "Remove Business Unit";
            const confirmStore = Alpine.store('confirm_modal');
            const roleStore = Alpine.store('bu_user');
            let role = roleStore.getCurrentDataByIndex(idx);
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Business Unit ${role.id} - ${role.id}</b> ?`, "", "remove", `${btn}`,
                () => {
                    confirmStore.hide();
                    removeBu(role);
                }
        ...
        removeAllRow(idx) {
            // console.log(idx, this.$el.checked);
            let act = "<b class='text-red-700'>remove</b>";
            let btn = "Remove Selected Business Unit";
            ...
            if (role.length > 0) {
                const confirmStore = Alpine.store('confirm_modal');
                confirmStore.show(
                    `${btn}`, `Are you sure you want to ${act} <b>Selected Business Unit</b> ?`, "", "remove", `${btn}`,
                    () => {
                        confirmStore.hide();
                        removeMultiBU(role.map((v, k) => { return v.id }));
                    }
                );
            }
        }
    ...
    Alpine.data('create', () => ({
        ...
        emptyForm() {
            this.title = "Set New Business Unit";
            this.bu_msg = "";
            this.start_dt_msg = "";
            this.end_dt_msg = "";
        ...
        submitCreate() {
            ...
            if (current_data.main === "" || current_data.main === null) {
                current_data.main = 0;
            }
            // console.log(current_data);
            // this.hideModal();
            if (current_data.bu_id === "" || current_data.bu_id === null) {
                this.bu_msg = "Select Business Unit !!!";
            } else if (current_data.start_dt === "" || current_data.start_dt === null) {
                this.start_dt_msg = "Select Start Date !!!";
            } else if (current_data.end_dt === "" || current_data.end_dt === null) {
                this.end_dt_msg = "Select End Date !!!";
            } else {
                if (current_data.id !== null) {
                    console.log(current_data);
                    updateBu(current_data);
                    this.cancelCreate();
                } else {
        ...
        showEdit(idx) {
            const role_dataStore = Alpine.store('bu_user');
            let _data = _.cloneDeep(role_dataStore.getCurrentDataByIndex(idx));
            // console.table(_data);
            this.title = "Edit Business Unit - " + _data.id;
            this.data = {
                id: _data.id,
                bu_id: _data.bu_id,
                code_bu: _data.code_bu,
                end_dt: _data.end_dt,
                full_name: _data.full_name,
                main: _data.main,
                name_bu: _data.name_bu,

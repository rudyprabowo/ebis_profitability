function reloadData(param) {
    ...
    console.log("TES");
    fetchWithTimeout(
        "/core-admin/mapping-user/bu-user/getall?dbcache=0",
        {
            // mode: "no-cors",
            // redirect: "follow",
            method: 'POST',
        ...
        .then((json) => {
            if (json.ret) {
                ...
                const roleStore = Alpine.store('bu_user');
...
function setNewBu(param) {
    console.log(param);
    const mainLoader = Alpine.store('loader');
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/mapping-user/bu-user/create",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                ...
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Set Script`, `Success <b>Set Business Unit</b>`, "", "success", `CLOSE`, () => {
                    notifStore.hide();
                ...
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Set Business Unit`, `Failed <b>Set Business Unit</b>`, json.data.msg, "failed", `CLOSE`, () => {
                    notifStore.hide();
                    reloadData(param.user_id);
                });
            } else {
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Set Business Unit`, `Failed <b>set Business Unit</b>, please try again`, json.data.msg, "failed", `CLOSE`, () => {
                    notifStore.hide();
                });
            }
        })
        .catch((error) => {
            ...
            mainLoader.hide();
            const notifStore = Alpine.store('notif_modal');
            notifStore.show(`Set Business Unit`, `Failed <b>set Business Unit</b>, please try again`, "", "failed", `CLOSE`, () => {
                notifStore.hide();
            });
        });
};

function updateBu(param) {
    // console.log("update");

    const mainLoader = Alpine.store('loader');
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/mapping-user/bu-user/update",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                ...
                // }));
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Update Business Unit`, `Success <b>Update Business Unit</b>`, "", "success", `CLOSE`, () => {
                    notifStore.hide();
                    reloadData(param.user_id);
                });
            } else if (json.data.code !== undefined && json.data.code === 1) {
                window.dispatchEvent(new CustomEvent('create-name-exist', { detail: json.data.msg }));
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Update Business Unit`, `Failed <b>Update Business Unit</b>`, json.data.msg, "failed", `CLOSE`, () => {
                    notifStore.hide();
                    reloadData(param.user_id);
                });
            } else {
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Update Business Unit`, `Failed <b>update Business Unit</b>, please try again`, "", "failed", `CLOSE`, () => {
                    notifStore.hide();
                });
            }
        })
...
function removeBu(param) {
    const mainLoader = Alpine.store('loader');
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/mapping-user/bu-user/delete",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                ...
                // }));
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Delete Business Unit`, `Success <b>Delete Business Unit</b>`, "", "success", `CLOSE`, () => {
                    notifStore.hide();
                    // reloadData(param.user);
                });
            } else {
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Remove Business Unit`, `Failed <b>remove Business Unit</b>, please try again`, "", "failed", `CLOSE`, () => {
                    notifStore.hide();
                });
            }
        })
...
function removeMultiBU(id) {
    // console.log(id);
    const mainLoader = Alpine.store('loader');
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/mapping-user/bu-user/deletemulti",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
...
function updatestatusBu(param) {
    // console.log("update");

    const mainLoader = Alpine.store('loader');
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/mapping-user/bu-user/updatestatus",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                ...
                // }));
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Update status Business Unit`, `Success <b>update status Business Unit</b>`, "", "success", `CLOSE`, () => {
                    notifStore.hide();
                });
            } else if (json.data.code !== undefined && json.data.code === 1) {
                window.dispatchEvent(new CustomEvent('create-name-exist', { detail: json.data.msg }));
                ...
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Update status Business Unit`, `Failed <b>update status Business Unit</b>, please try again`, "", "failed", `CLOSE`, () => {
                    notifStore.hide();
                });
            }
        })

function reloadData(dbcache=1) {
  ...
  fetchWithTimeout(
    "/core-admin/manage-script/module/getall?dbcache="+dbcache,
    {
      // mode: "no-cors",
      // redirect: "follow",
      method: 'POST',
...
function addModule(param) {
  ...
  fetchWithTimeout(
    "/core-admin/manage-script/module/create",
    {
      method: 'POST',
      headers: {
        Accept: 'application/json',
    ...
    .then((json) => {
      ...
      if (json.ret && json.data.code!==undefined && json.data.code===0) {
        ...
        window.dispatchEvent(new CustomEvent('add-notif', {
          detail: { type:"success", title: "Add Module", msg: `Success add <b>Module ${param.name}</b>`, timeout:5000 }
        }));
        // const notifStore = Alpine.store('notif_modal');
        // notifStore.show(`Add Module`, `Success <b>add module</b>`, "", "success", `CLOSE`, () => {
        //   notifStore.hide();
        // });
        ...
        const notifStore = Alpine.store('notif_modal');
        notifStore.show(`Add Module`, `Failed <b>add module</b>, please try again`, "", "failed", `CLOSE`, () => {
          notifStore.hide();
        });
      }
    })
...
function changeStatus(param,el,old_val) {
  ...
  mainLoader.show("sending request...");
  fetchWithTimeout(
    "/core-admin/manage-script/module/updatestatus",
    {
      method: 'POST',
      headers: {
        Accept: 'application/json',
    ...
    .then((json) => {
      ...
      if (json.ret && json.data.code!==undefined && json.data.code===0) {
        ...
        el.checked = !old_val;
        window.dispatchEvent(new CustomEvent('add-notif', {
          detail: { type:"success", title: "Update Module Status", msg: `Success update <b>Module ${param.id} - ${param.name}</b> status`, timeout:5000 }
        }));
        // const notifStore = Alpine.store('notif_modal');
        ...
        const notifStore = Alpine.store('notif_modal');
        notifStore.show(`Update Module Status`, `Failed <b>update module status</b>, please try again`, "", "failed", `CLOSE`, () => {
          notifStore.hide();
        });
      }
    })
...
function removeModule(param) {
  const mainLoader = Alpine.store('loader');
  mainLoader.show("sending request...");
  fetchWithTimeout(
    "/core-admin/manage-script/module/delete",
    {
      method: 'POST',
      headers: {
        Accept: 'application/json',
    ...
    .then((json) => {
      ...
      if (json.ret && json.data.code !== undefined && json.data.code === 0) {
        const moduleStore = Alpine.store('module');
        let _data = _.cloneDeep(moduleStore.data);
        _data = _.filter(_data, (v, k) => {
          return v.id!==param.id;
        });
        moduleStore.setData(_data);
        filterData();
        window.dispatchEvent(new CustomEvent('add-notif', {
          detail: { type:"success", title: "Remove Module", msg: `Success remove  <b>Module ${param.id} - ${param.name}</b>`, timeout:5000 }
        }));
        // const notifStore = Alpine.store('notif_modal');
...
function removeMultiModule(id) {
  ...
  mainLoader.show("sending request...");
  fetchWithTimeout(
    "/core-admin/manage-script/module/deletemulti",
    {
      method: 'POST',
      headers: {
        Accept: 'application/json',
    ...
    .then((json) => {
      ...
      if (json.ret && json.data.code !== undefined && json.data.code === 0) {
        ...
        filterData();
        window.dispatchEvent(new CustomEvent('add-notif', {
          detail: { type:"success", title: "Remove Module", msg: `Success <b>remove selected module</b>`, timeout:5000 }
        }));
        // const notifStore = Alpine.store('notif_modal');
        // notifStore.show(`Remove Module`, `Success <b>remove module</b>`, "", "success", `CLOSE`, () => {
        //   notifStore.hide();

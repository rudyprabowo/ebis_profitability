document.addEventListener('alpine:init', () => {
    Alpine.data('table', () => ({
        ...
        bind: {
            ...
            check_all: {
                ['@click']() {
                    ...
                    const layoutStore = Alpine.store('menu_role');
        ...
        changeStatus(idx) {
            ...
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Menu ${role.id} - ${role.menu_name}</b> ?`, "", "info", `${btn}`,
                () => {
                    this.$el.checked = !old_val;
                    confirmStore.hide();
        ...
        removeRow(idx) {
            ...
            let role = roleStore.getCurrentDataByIndex(idx);
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Menu ${role.id} - ${role.role}</b> ?`, "", "remove", `${btn}`,
                () => {
                    confirmStore.hide();
                    removeMenu(role);
                }
        ...
        removeAllRow(idx) {
            ...
            if (role.length > 0) {
                ...
                confirmStore.show(
                    ...
                    () => {
                        confirmStore.hide();
                        removeMultiMenu(role.map((v, k) => { return v.id }));
                    }
                );
            }
        }
    ...
    Alpine.data('create', () => ({
        ...
        submitCreate() {
            ...
            // this.hideModal();
            if (current_data.menu_id === "" || current_data.menu_id === null) {
                this.menu_msg = "Select Menu !!!";
            } else {
        ...
        showEdit(idx) {
            const role_dataStore = Alpine.store('menu_role');
    ...
    Alpine.data('upload', () => ({
        ...
        ajax: {
            url: "/core-admin/manage-role/menu-role/upload",
            method: "POST",
            headers: {
                Accept: 'application/json',
                'Content-type': 'application/json'
            },
            body: JSON.stringify({param:_id_role}),

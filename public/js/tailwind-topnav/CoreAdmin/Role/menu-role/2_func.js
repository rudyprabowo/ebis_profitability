function reloadData(param) {
    ...
    console.log("TES");
    fetchWithTimeout(
        "/core-admin/manage-role/menu-role/getall?dbcache=0",
        {
            // mode: "no-cors",
            // redirect: "follow",
            method: 'POST',
        ...
        .then((json) => {
            if (json.ret) {
                ...
                const roleStore = Alpine.store('menu_role');
...
function filterData(print = false) {
    ...
    const moduleStore = Alpine.store('menu_role');
...
function setNewMenu(param) {
    const mainLoader = Alpine.store('loader');
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/manage-role/menu-role/create",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                ...
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Set Menu`, `Success <b>Set Menu</b>`, "", "success", `CLOSE`, () => {
                    notifStore.hide();
                ...
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Set Menu`, `Failed <b>Set Menu</b>`, json.data.msg, "failed", `CLOSE`, () => {
                    notifStore.hide();
                    reloadData(param.role_id);
                });
            } else {
...
function updateMenu(param) {
    ...
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/manage-role/menu-role/update",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                ...
                // }));
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Update Menu`, `Success <b>Update Menu</b>`, "", "success", `CLOSE`, () => {
                    notifStore.hide();
                    reloadData(param.role_id);
                });
            } else if (json.data.code !== undefined && json.data.code === 1) {
                window.dispatchEvent(new CustomEvent('create-name-exist', { detail: json.data.msg }));
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Update Menu`, `Failed <b>Update Menu</b>`, json.data.msg, "failed", `CLOSE`, () => {
                    notifStore.hide();
                    reloadData(param.role_id);
                });
            } else {
...
function removeMenu(param) {
    ...
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/manage-role/menu-role/delete",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                ...
                // }));
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Delete Menu`, `Success <b>Delete Menu</b>`, "", "success", `CLOSE`, () => {
                    notifStore.hide();
...
function removeMultiMenu(id) {
    ...
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/manage-role/menu-role/deletemulti",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
...
function updatestatusMenu(param) {
    ...
    mainLoader.show("sending request...");
    fetchWithTimeout(
        "/core-admin/manage-role/menu-role/updatestatus",
        {
            method: 'POST',
            headers: {
                Accept: 'application/json',
        ...
        .then((json) => {
            ...
            if (json.ret && json.data.code !== undefined && json.data.code === 0) {
                ...
                // }));
                const notifStore = Alpine.store('notif_modal');
                notifStore.show(`Update status Menu`, `Success <b>update status Menu</b>`, "", "success", `CLOSE`, () => {
                    notifStore.hide();
                });
            } else if (json.data.code !== undefined && json.data.code === 1) {
                window.dispatchEvent(new CustomEvent('create-name-exist', { detail: json.data.msg }));

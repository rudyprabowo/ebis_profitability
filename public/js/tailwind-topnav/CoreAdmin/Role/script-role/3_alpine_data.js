document.addEventListener('alpine:init', () => {
    Alpine.data('table', () => ({
        ...
        bind: {
            ...
            check_all: {
                ['@click']() {
                    ...
                    const layoutStore = Alpine.store('script_role');
            ...
            row_checkbox: {
                ['@click']() {
                    ...
                    let checked = this.$el.checked;
                    const roleStore = Alpine.store('script_role');
        ...
        init() {
           // reloadData();
           this._list_role = list_role;
           this._rid = rid;
           if (this._rid !== null) {
               reloadData(this._rid);
           }
        },
        SelectDataMenu() {
            // console.log(this.id_user);
            reloadData(this.id_role);
            _id_role = this.id_role;
        },
        changeSorting(field) {
        ...
        changeStatus(idx) {
            ...
            let btn = "Activate Script";
            if (!this.$el.checked) {
                act = "<b class='text-red-700'>deactivate</b>";
                btn = "Deactivate Script";
            }
            ...
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Script ${role.id} - ${role.menu_name}</b> ?`, "", "info", `${btn}`,
                () => {
                    this.$el.checked = !old_val;
                    confirmStore.hide();
        ...
        removeRow(idx) {
            ...
            let btn = "Remove Script";
            const confirmStore = Alpine.store('confirm_modal');
            const roleStore = Alpine.store('script_role');
            let role = roleStore.getCurrentDataByIndex(idx);
            confirmStore.show(
                `${btn}`, `Are you sure you want to ${act} <b>Script ${role.id} - ${role.role}</b> ?`, "", "remove", `${btn}`,
                () => {
                    confirmStore.hide();
                    removeScript(role);
                }
        ...
        removeAllRow(idx) {
            // console.log(idx, this.$el.checked);
            let act = "<b class='text-red-700'>remove</b>";
            let btn = "Remove Selected Script";
            ...
            if (role.length > 0) {
                const confirmStore = Alpine.store('confirm_modal');
                confirmStore.show(
                    `${btn}`, `Are you sure you want to ${act} <b>Selected Script</b> ?`, "", "remove", `${btn}`,
                    () => {
                        confirmStore.hide();
                        removeMultiScript(role.map((v, k) => { return v.id }));
                    }
                );
            }
        }
    ...
    Alpine.data('create', () => ({
        ...
        submitCreate() {
            ...
            // this.hideModal();
            if (current_data.module_id === "" || current_data.module_id === null) {
                this.module_msg = "Select Module !!!";
            } else if (current_data.controller_id === "" || current_data.controller_id === null) {
                this.controller_msg = "Select Controller !!!";
            } else if (current_data.action_id === "" || current_data.action_id === null) {
                this.action_msg = "Select Action !!!";
            } else if (current_data.layout_id === "" || current_data.layout_id === null) {
                this.layout_msg = "Select Layout !!!";
            } else {
                if (current_data.id !== null) {
                    updateScript(current_data);
                    this.cancelCreate();
                } else {
                    current_data = _.pickBy(current_data, (val, idx) => {
                        return idx !== "id";
                    });
                    current_data.role_id = _id_role;
                    // console.log(current_data);
                    setNewScript(current_data);
                    this.cancelCreate();
                }
            }
        },
        ...
        showEdit(idx) {
            const role_dataStore = Alpine.store('script_role');
